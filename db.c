...
CLOUD_SOURCE_DATA * cloud_source_list;
CLOUD_INDEX_DATA * cloud_index_list;
CLOUD_INSTANCE_DATA * cloud_instance_list;
...
void save_warscore(void)
{
    ...
    sprintf(strsave, "%s", WARSCORE_FILE);
    ...
    fprintf(fp, "%d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d",
	    temp->tier_legends_lost,
	    temp->tier_lost,
	    temp->tier_legends_killed,
	    temp->tier_killed,
	    temp->tier_raids,
	    temp->tier_been_raided,
	    temp->tier_warscore,
	    temp->kael_legends_lost,
	    temp->kael_lost,
	    temp->kael_legends_killed,
	    temp->kael_killed,
	    temp->kael_raids,
	    temp->kael_been_raided,
	    temp->kael_warscore,
	    temp->um_legends_lost,
	    temp->um_lost,
	    temp->um_legends_killed,
	    temp->um_killed,
	    temp->um_raids,
	    temp->um_been_raided,
	    temp->um_warscore,
	    temp->cairn_legends_lost,
	    temp->cairn_lost,
	    temp->cairn_legends_killed,
	    temp->cairn_killed,
	    temp->cairn_raids,
	    temp->cairn_been_raided,
	    temp->cairn_warscore
	);
    fclose(fp);
    return;
...
void load_warscore(void)
{
    ...
    main_warscore = malloc(sizeof(struct warscore_data));

    main_warscore->tier_legends_lost = fread_number(fp);
    main_warscore->tier_lost = fread_number(fp);
    main_warscore->tier_legends_killed = fread_number(fp);
    main_warscore->tier_killed = fread_number(fp);
    main_warscore->tier_raids = fread_number(fp);
    main_warscore->tier_been_raided = fread_number(fp);
    main_warscore->tier_warscore = fread_number(fp);

    main_warscore->kael_legends_lost = fread_number(fp);
    main_warscore->kael_lost = fread_number(fp);
    main_warscore->kael_legends_killed = fread_number(fp);
    main_warscore->kael_killed = fread_number(fp);
    main_warscore->kael_raids = fread_number(fp);
    main_warscore->kael_been_raided = fread_number(fp);
    main_warscore->kael_warscore = fread_number(fp);

    main_warscore->um_legends_lost = fread_number(fp);
    main_warscore->um_lost = fread_number(fp);
    main_warscore->um_legends_killed = fread_number(fp);
    main_warscore->um_killed = fread_number(fp);
    main_warscore->um_raids = fread_number(fp);
    main_warscore->um_been_raided = fread_number(fp);
    main_warscore->um_warscore = fread_number(fp);

    main_warscore->cairn_legends_lost = fread_number(fp);
    main_warscore->cairn_lost = fread_number(fp);
    main_warscore->cairn_legends_killed = fread_number(fp);
    main_warscore->cairn_killed = fread_number(fp);
    main_warscore->cairn_raids = fread_number(fp);
    main_warscore->cairn_been_raided = fread_number(fp);
    main_warscore->cairn_warscore = fread_number(fp);

    fclose(fp);
    return;
}
/*zz
void load_topplayers(void)
{
    FILE *players, *current_player;
    TOPPLAYER_DATA *temp, *warlist;
    char *word;
    char *name = "Someone";
    char *desc = "";
    int side = 0;
    ...
    bool done, is_inserted, is_modified;

    sprintf(log_buf, "Generating Topplayer Lists.");
    log_string(log_buf);


    // Create a topplayer file from a list of existing players.
    

    players = popen("echo #PLAYERLISTING > ../player/playerfile", "r");
    pclose(players);
    players = popen("ls -1 --color=no ../player/ >> ../player/playerfile", "r");
    pclose(players);
    players = popen("echo \\~ >> ../player/playerfile", "r");
    pclose(players);

    players = fopen("../player/playerfile", "r");
    for (;;) {
	fread_to_eol(players);
	word = feof(players) ? "~" : fread_word(players);

	if (!str_cmp(word, "playerfile"))
	    continue;
	...
	current_player = fopen(word, "r");
	done = FALSE;

	for (;;) {
	    fread_to_eol(current_player);
	    word = feof(current_player) ? "End" : fread_word(current_player);

	    switch (word[0]) {
            case 'D':
                if (!str_cmp(word, "Desc"))
                    desc = fread_string(current_player);
                break;  

	    case 'V':
		if (!str_cmp(word, "Vers"))
		    version = fread_number(current_player);
		break;
	    case 'N':
		if (!str_cmp(word, "Name"))
		    name = fread_string(current_player);
//
#ifdef chase_corrupted
log_string(name);
#endif
		break;
	    case 'R':
		if (!str_cmp(word, "Race"))
		    race = race_lookup(fread_string(current_player));
		break;
	    case 'S':
		if (!str_cmp(word, "Side")){
		    if (version == 5) {
			side = fread_number(current_player);
			if (side == 0)
			    side = SIDE_TIER;
			else if (side == 1)
			    side = SIDE_KAEL;
			...
			char *temp;
			temp = fread_string(current_player);
...
//side = fread_number(current_player)
		...
		if (!str_cmp(word, "Warpoints"))
		    warp = fread_number(current_player);
		break;
	    case 'C':
		if (!str_cmp(word, "Cla")) {
		    if (version == 6) {
			...
			temp = fread_string(current_player);
			clas = 0;
			while (str_cmp(class_table[clas].name, temp))
			    clas++;
		    } else if (version == 5) {
			clas = fread_number(current_player);
		    }
		}
// clas = fread_number(current_player)         
		...
		if (!str_cmp(word, "Playerkilled"))
		    pkil = fread_number(current_player);
		break;
	    case 'H':
		if (!str_cmp(word, "HMV"))
		    done = TRUE;
	...
	for (temp = main_warlist; temp != NULL; temp = temp->next) {
	    ...
	    if (((warp - pkil) > (temp->warpoints - temp->playerkilled)) && !is_inserted && !is_modified) {
		warlist = new_warlist();
		warlist->next = temp;
		...
		temp->prev = warlist;
		warlist->name = strdup(name);
		warlist->warpoints = warp;
		warlist->kingdom = side;
		warlist->race = race;
		warlist->class = clas;
		warlist->playerkilled = pkil;
		is_inserted = TRUE;
		is_modified = TRUE;
	    }
    ...
    players = popen("rm -f ../player/playerfile", "r");
    pclose(players);
...
void save_topplayers(void)
{
    ...
    sprintf(strsave, "%s", TOPPLAYER_FILE);
 ...
 */
//void boot_db(void) //Izlude copyover change
void boot_db() //Izlude copyover change
{
    ...
    sprintf(log_buf, "Booting ActofWar Chapter 3.");
    log_string(log_buf);

    sprintf(log_buf, "Allocating Data Space.");
    log_string(log_buf);
    ...
    sprintf(log_buf, "Initializing random number generator.");
    log_string(log_buf);
    ...
    sprintf(log_buf, "Setting Time and Weather.");
    log_string(log_buf);
    ...
    sprintf(log_buf, "Assigning gsn's for skills.");
    log_string(log_buf);
    ...
    sprintf(log_buf, "Reading in the Area Files.");
    log_string(log_buf);
	...
	for (;;) {
		...
		if ((fpArea = fopen(strArea, "r")) == NULL) {
		    ...
		    sprintf(log_buf, "Opening Area %s", strArea);
		    log_string(log_buf);
		}
	    }
	...
	sprintf(log_buf, "Initializing main_warlist.");
	log_string(log_buf);

	main_warlist = new_warlist();
	main_warlist->prev = NULL;
	temp = main_warlist;

	for (list = 0; list < (MAX_TOPPLAYERS - 1); list++) {
	    main_warlist->next = new_warlist();
	    main_warlist->next->prev = main_warlist;
	    main_warlist = main_warlist->next;
	    main_warlist->next = NULL;
	}
	main_warlist = temp;
	...
	log_string(log_buf);
	fix_exits();
	sprintf(log_buf, "Fixing Mobprogs.");
	log_string(log_buf);
	fix_mobprogs();
	fBootDb = FALSE;
	sprintf(log_buf, "Converting Objects.");
	log_string(log_buf);
	convert_objects();	/* ROM OLC */
	sprintf(log_buf, "Reseting Areas.");
	log_string(log_buf);
	area_update();
	sprintf(log_buf, "Loading the Note System.");
	log_string(log_buf);
	load_notes();
	sprintf(log_buf, "Loading Illegal Names.");
	log_string(log_buf);
	load_illegal();
	sprintf(log_buf, "Loading the Ban File.");
	log_string(log_buf);
	load_bans();
	sprintf(log_buf, "Loading Songs.");
	log_string(log_buf);
	load_songs();
        log_string("Initializing skills...");
//	loadskills();

//	load_topplayers();
	sprintf(log_buf, "Generating Warscore.");
	log_string(log_buf);
	load_warscore();
	sprintf(log_buf, "Loading Crash Saved Objects");
	log_string(log_buf);
	//load_saved_objects();
	/*Disable Added*/
      load_disabled(); 
	sprintf(log_buf, "Loading Disabled Commands List");
	log_string(log_buf);
	sprintf(log_buf, "Success?");
	log_string(log_buf);
    }
...
void load_helps(FILE * fp)
{
	...
	if (!str_cmp(pHelp->keyword, "greeting1") || !str_cmp(pHelp->keyword, "{Cgreeting1"))
	...
	while (cHelp != NULL && (0 <
				 strcmp((pHelp->keyword[0] == '{') ? pHelp->keyword + 2 : pHelp->keyword,
					(cHelp->keyword[0] == '{') ? cHelp->keyword + 2 : cHelp->keyword))) {
	    help_last = cHelp;
	    cHelp = cHelp->next;
	}
...
void load_rooms(FILE * fp)
{
	...
	pRoomIndex->trap = alloc_perm(sizeof(*pRoomIndex->trap));
	...
	if (pRoomIndex->sector_type == SECT_INSIDE || pRoomIndex->sector_type == SECT_UNDERGROUND)
...
//      room_update ( pArea );  
	...
	if ((!pArea->empty && (pArea->nplayer == 0 || pArea->age >= 10))
	    || pArea->age >= 25) {
	 // Vashna 07-11-01: Commented out because of it being an unused variable.
         // ROOM_INDEX_DATA *pRoomIndex;
...
//	    pRoomIndex = get_room_index(20000);
//	    if (pRoomIndex != NULL && pArea == pRoomIndex->area)
//		pArea->age = 10 - 2;
//	    else if (pArea->nplayer == 0)
//		pArea->empty = TRUE;

//	    pRoomIndex = get_room_index(20100);
//	    if (pRoomIndex != NULL && pArea == pRoomIndex->area)
//		pArea->age = 10 - 2;
//	    else if (pArea->nplayer == 0)
//		pArea->empty = TRUE;
//****
//	    pRoomIndex = get_room_index(20300);
//	    if (pRoomIndex != NULL && pArea == pRoomIndex->area)
//		pArea->age = 10 - 2;
//	    else if (pArea->nplayer == 0)
//		pArea->empty = TRUE;

//	    pRoomIndex = get_room_index(27900);
//	    if (pRoomIndex != NULL && pArea == pRoomIndex->area)
//		pArea->age = 10 - 2;
//	    else if (pArea->nplayer == 0)
//		pArea->empty = TRUE;

//	    pRoomIndex = get_room_index(20200);
//	    if (pRoomIndex != NULL && pArea == pRoomIndex->area)
//		pArea->age = 10 - 2;
//	    else if (pArea->nplayer == 0)
//		pArea->empty = TRUE;

//	    pRoomIndex = get_room_index(20400);
//	    if (pRoomIndex != NULL && pArea == pRoomIndex->area)
//		pArea->age = 10 - 2;
//	    else if (pArea->nplayer == 0)
//		pArea->empty = TRUE;
...
CHAR_DATA *create_mobile(MOB_INDEX_DATA * pMobIndex)
{
	...
	mob->hit = mob->hit * 3 / 2;
	mob->damage[DICE_NUMBER] =mob->damage[DICE_NUMBER] * 3 / 2;
	mob->dam_type = mob->dam_type * 3 / 2;
...
void clone_mobile(CHAR_DATA * parent, CHAR_DATA * clone)
{
    ...
    clone->dir_stack = parent->dir_stack;
...
OBJ_DATA *create_object(OBJ_INDEX_DATA * pObjIndex, int level)
{
  ...
  if( obj->cost > 100000) obj->cost = 100000;
...
void do_areas(CHAR_DATA * ch, char *argument)
{
    ...
    send_to_char("Use the map feature to get around.\n\r Type 'map'.\n\r", ch);
    return;
...
{
    return value_00 + level * (value_32 - value_00) / 50;
}
...
void writeskills( void )
{
	FILE * fp;
	int iClass, group, sn, spell;

	if((fp = fopen(SKILLS_FILE,"w"))==NULL)
	{
		bug("Writeskills: Cannot open SKILLS_FILE to write",0);
		exit(1);
	}
	fprintf(fp, "#P~\n\n");


	for (spell = 0; spell < MAX_SKILL; spell++)
	{
		if (skill_table[spell].name == NULL)
			continue;
		fprintf(fp, "#Spell~\n");
		fprintf(fp, "%s~\n", skill_table[spell].name);
		fprintf(fp, "Man~ %d\n", skill_table[spell].min_mana);
		fprintf(fp, "Lag~ %d\n\n", skill_table[spell].beats);

		fprintf(fp, "Cp~\n");
		for (iClass = 0; iClass < MAX_CLASS; iClass++)
			if (skill_table[spell].rating[iClass] > 0)
				fprintf(fp, "%s~ %d\n",
				class_table[iClass].name, skill_table[spell].rating[iClass]);
		fprintf(fp, "EndCp~\n\n");
		fprintf(fp, "Level~\n");
		for (iClass = 0; iClass < MAX_CLASS; iClass++)
			if (skill_table[spell].skill_level[iClass] > 0)
				fprintf(fp, "%s~ %d\n",
				class_table[iClass].name, skill_table[spell].skill_level[iClass]);
		fprintf(fp, "EndLevel~\n");
	...
	for (group = 0; group_table[group].name != NULL; group++)
	{
		fprintf(fp, "#Group~\n");
		fprintf(fp, "%s~\n\n", group_table[group].name);
		fprintf(fp, "Cp~\n");
		for (iClass = 0; iClass < MAX_CLASS; iClass++)
			if(group_table[group].rating > 0)
				fprintf(fp, "%s~ %d\n", class_table[iClass].name, group_table[group].rating[iClass]);
		fprintf(fp, "EndCp~\n\n");

		fprintf(fp, "Sk~\n");
		for (sn = 0; group_table[group].spells[sn] != NULL; sn++)
			fprintf(fp, "%s~\n", group_table[group].spells[sn]);
		fprintf(fp, "EndSk~\n");
...
/* The following also goes in db.c
 *
 * -Tasydra
 */

void read_spells(FILE *fp)
{
	int sn, iClass;
	char letter;
	char *word;
...
//				skill_table[sn].beats = fread_number(fp);
				...
				while (str_cmp((fclass = fread_string(fp)), "EndCp") )
				{
					iClass = class_lookup(fclass);
//					skill_table[sn].rating[iClass] = fread_number(fp);
				...
				while (str_cmp((fclass = fread_string(fp)), "EndLevel") )
				{
					iClass = class_lookup(fclass);
//					skill_table[sn].skill_level[iClass] = fread_number(fp);
				...
				free_string(fclass);
				free_string(fread_string(fp));
			} else if (!str_cmp(word, "EP") )
			{
				free_string(word);
				break;
			}
		} else {
			bug("Error reading spells!", 0);
			return;
		}
		free_string(word);
	}
	...
	for (;;)
	{
		...
		if (letter == '#')
		{
			...
			if (!str_cmp(word, "Group") )
			{
				...
				{
					iClass = class_lookup(fclass);
//					group_table[sn].rating[iClass] = fread_number(fp);
					free_string(fclass);
				}
				...
				while (str_cmp((fclass = fread_string(fp)), "EndSk") )
				{
//					group_table[sn].spells[x] = fclass;
					free_string(fclass);
					x++;
				}
				x = 0;
				free_string(fclass);
				free_string(fread_string(fp));
			} else if (!str_cmp(word, "ER") )
			{
				free_string(word);
				break;
			}
		} else {
			bug("Error reading groups!", 0);
			return;
		}
		free_string(word);
	}
	...
	for(sn = 0;sn < MAX_SKILL;sn++)
		{
			for(class = 0;class < MAX_CLASS;class++)
				{
//					skill_table[sn].skill_level[class] = 0;
  //                  skill_table[sn].rating[class] = 0;
                }
        }
...

{
	...
	{
		bug("Loadskills: Cannot open SKILLS_FILE",0);
		return;
	}
	...
	for(;;)
	{
		...
		if (letter == '#')
		{
			...
			if (!str_cmp(word, "P") )
				read_spells(fp);
			else if (!str_cmp(word, "R") )
				read_groups(fp);
			else if (!str_cmp(word, "EOF") )
				break;
		} else {
			bug("Error loading SKILLS_FILE!", 0);
			exit(1);
			break;
		}
		free_string(word);
	...
	for(class = 0;class < MAX_CLASS;class++)
//	skill_table[0].skill_level[class] = LEVEL_HERO + 1;
