...
DESCRIPTOR_DATA *new_descriptor(void)
{
    ...
    *d = d_zero;
    ...
    d->connected	= CON_COLOR;
    d->showstr_head	= NULL;
    d->showstr_point = NULL;
    d->outsize	= 2000;
    d->outbuf	= alloc_mem( d->outsize );
  //end copyover changes
...
TOPPLAYER_DATA *new_warlist(void)
{
    TOPPLAYER_DATA *list;

    list = alloc_mem(sizeof(TOPPLAYER_DATA));

    list->name = strdup("Nobody");
    list->race = 0;
    list->kingdom = 0;
    list->level = 0;
    list->warpoints = 0;
...
void free_warlist(TOPPLAYER_DATA * list)
{
}
...
void free_char(CHAR_DATA * ch)
{
    ...
    for (obj = ch->storage; obj != NULL; obj = obj_next) {
	obj_next = obj->next_content;
	extract_obj(obj);
    }
	...
	//ch->pcdata->ignoring = char_free;

    while (ch->dir_stack != NULL)
	pop_dir(ch);
...
void free_pcdata(PC_DATA * pcdata)
{
    ...
    free_string(pcdata->private_topic);
    free_string(pcdata->prename);
