...
/* ROM specific bits: */
    ...
    {"sclass", shield_class, "Shield class."},
    {NULL, NULL, NULL}
};
...
void show_flag_cmds(CHAR_DATA * ch, const struct flag_type *flag_table)
{
	...
	if (flag_table[flag].settable) {
		 if (flag_table[flag].name == sector_flags[flag].name) {
		switch (flag) {
		case SECT_INSIDE:
			 strcat(buf1, "{W# ");
			 break;
		...
		case SECT_CAVE:
			 ...
			 break;
		case SECT_FARMLAND:
			 strcat(buf1, "{y: ");/*Vashna 04-05-01*/
			 break;               /*New sectors added*/
		case SECT_CAVERN:
			 strcat(buf1, "{DO ");
			 break;
		case SECT_BRIDGE:
		...
		default:
		 ...
		 sprintf(buf, "%-17.16s", flag_table[flag].name);
		 strcat(buf1, buf);
		 if (++col % 4 == 0)
		strcat(buf1, "{x\n\r");
	}
...
const struct lookup_sect tab_sector_types[] =
{
  ...
  { "tundra",                  16, 50, "{c="},
  { "wasteland",               17, 50, "{yo"},
  { "dense_forest",            18, 50, "{g#"},
  { "lava",                    19, 50, "{R~"},
  { "underground",             20, 50, "{m="}, /* moo */
  { "bog",                     21, 50, "{g~"},
  { "ruins",                   22, 50, "{R#"},
  { "house",                   23, 50, "{rH"},
  { "pyramid",                 24, 50, "{Y^"},
  ...
  { "frozen_sea",              36, 50, "{W~"},
  { "dock",                    37, 50, "{y%"},
  { "ice_mountain",            38, 50, "{W^"},
  { "redocean",                39, 50, "{R&"},
  { "frozen_forest",           40, 50, "{W@"},
  { "dirtpath",                41, 50, "{y~"},
  { "wheatfield",              42, 50, "{Y!"},
  { "darkwood",                43, 50, "{D@"},
  { "wind",                    44, 50, "{C#"},
...
const struct lookup_type tab_room_flags[] =
{
  { "nada",			   0,      0 },
  { "dark",                A,      1000},
  { "no_mob",              C,      4000},   /* increased regen in this room */
  { "indoors",             D,      3000},
  { "private",             J,      500},
  { "safe",                K,     5000},
  { "solitary",            L,     1000},  /* players lose hp each tick   */
  { "pet_shop",            M,     1000},  /* players lose hp each tick   */
  { "no_recall",           N,    200},  /* players may freely pk here  */
  { "imp_only",            O,    200},  /* room is quiet               */
  { "gods_only",           P,    1000},
  { "heroes_only",         Q,   4000},
  { "newbies_only",        R,   1000},
  { "law",                 S,   0},
...
char * show_values(const struct lookup_type * table,int value, bool fBit)
{
     ...
     for (a=0;table[a].text!=NULL;a++)
     {
         ...
         && ( strcmp(table[a].text,"placeholder") )  )
      {
       strcat(buf,"      ");
       sprintf( tmp, "%s%-13s",
	 fBit?  (IS_SET( value, table[a].value ) ? "{Y*" : "{n ") :
		(value == table[a].value ? "{Y*" : "{n "),
       table[a].text );
       strcat(buf,tmp );
       if ( ++foo % 4 == 0 )
	  strcat(buf,"\n\r");
...
void autobuild_exit(CHAR_DATA * ch, int newRNum, int door)
{
    AREA_DATA *pArea;
    ROOM_INDEX_DATA *pRoom;
    int value;
    ...
    /* check if room already is their */
    if (get_room_index(newRNum)) {
    } else {			/* make a new room */
	...
	send_to_char("Autobuild: Room created you just entered.\n\r", ch);
    }
    /* build exit to the room */
    sprintf(buf, "link %d", newRNum);
    change_exit(ch, buf, door);
}

void autobuild(CHAR_DATA * ch, int door, ROOM_INDEX_DATA * pRoom)
{

    ROOM_INDEX_DATA *to_room, *in_room, *new_room;
    AREA_DATA *area;
    int curvnum = 0;
    ...
    if (!IS_NPC(ch))
     if ( (ch->desc->editor == ED_ROOM) && (IS_SET(ch->act, PLR_AUTOBUILD)) && (IS_IMMORTAL(ch)) )
		{
          ...
          ||   (  ( str_cmp( pexit->keyword, ""   ) ) ) )
             {
	           ...
	           for ( curvnum = area->min_vnum; curvnum <= area->max_vnum; curvnum++ )
			   {
			       ...
			       continue;
					 sprintf(buf, "%s dig %d", dir_name[door], curvnum);
                     redit(ch,buf);
					 send_to_char("{Y***{RAUTOBUILD IS ENABLED!{Y***{x\n\r", ch);
					 move_char(ch, door, FALSE);
					 new_room = ch->in_room;
					 free_string(new_room->name);
					 free_string(new_room->description);
					 new_room->name = str_dup(in_room->name);
					 new_room->description = str_dup(in_room->description);
					 new_room->room_flags = in_room->room_flags;
					 new_room->sector_type = in_room->sector_type;
					 new_room->heal_rate = in_room->heal_rate;
					 new_room->mana_rate = in_room->mana_rate;
					 have_rooms = TRUE;
					 break;
			   }
	         }
		   }
      if (!have_rooms && checked)
	  {
...
bool change_exit(CHAR_DATA * ch, char *argument, int door)
{
	...
	/* check flag for autobuild */
	if (IS_SET(ch->act, PLR_AUTOBUILD)) {
	    autobuild(ch, door, pRoom);
	}
...
{
    ROOM_INDEX_DATA *pCurrentRoom;
    ROOM_INDEX_DATA *pCopyRoom;
	...
	send_to_char("You do not have high enough security to rcopy.\n\r", ch);
	return;
    }
    argument = one_argument(argument, arg1);

    if (!is_number(arg1)) {
	send_to_char("Please use a valid room number to copy. \n\r", ch);
	return;
    }
    value = atoi(arg1);
    ...
    pCurrentRoom = ch->in_room;
    pCopyRoom = get_room_index(value);

    pCurrentRoom->name = str_dup(pCopyRoom->name);
    pCurrentRoom->description = str_dup(pCopyRoom->description);
    pCurrentRoom->room_flags = pCopyRoom->room_flags;
    pCurrentRoom->light = pCopyRoom->light;
    pCurrentRoom->sector_type = pCopyRoom->sector_type;
    pCurrentRoom->heal_rate = pCopyRoom->heal_rate;
    pCurrentRoom->mana_rate = pCopyRoom->mana_rate;

}

void do_mcopy(CHAR_DATA * ch, char *argument)
{
    MOB_INDEX_DATA *pCurrentMob;
    MOB_INDEX_DATA *pCopyMob;
    ...
    if (ch->pcdata->security < 9) {
	send_to_char("You do not have high enough security to mcopy.\n\r", ch);
	return;
    }
    argument = one_argument(argument, arg1);
    ...
    if (!is_number(arg1)) {
	send_to_char("Please use a valid SOURCE mobile number to COPY.\n\r", ch);
	return;
    }
    if (!is_number(arg2)) {
	send_to_char("Please use a valid TARGET mobile to copy TO.\n\r", ch);
	return;
    }
    value = atoi(arg1);
    value2 = atoi(arg2);
	...
	send_to_char("SOURCE mobile you are trying to copy does not exist. \n\r", ch);
	return;
    }
    if (!get_mob_index(value2)) {
	send_to_char("TARGET mobile you are copying TO does not exist. \n\r", ch);
	return;
    }
    pCurrentMob = get_mob_index(value2);
    pCopyMob = get_mob_index(value);

    pCurrentMob->player_name = str_dup(pCopyMob->player_name);
    pCurrentMob->short_descr = str_dup(pCopyMob->short_descr);
    pCurrentMob->long_descr = str_dup(pCopyMob->long_descr);
    pCurrentMob->description = str_dup(pCopyMob->description);

    pCurrentMob->sex = pCopyMob->sex;
    pCurrentMob->level = pCopyMob->level;
    pCurrentMob->act = pCopyMob->act;
    pCurrentMob->affected_by = pCopyMob->affected_by;
    pCurrentMob->alignment = pCopyMob->alignment;
    pCurrentMob->hitroll = pCopyMob->hitroll;
    pCurrentMob->race = pCopyMob->race;		/* - Hugin */
    pCurrentMob->form = pCopyMob->form;		/* ROM patch -- Hugin */
    pCurrentMob->parts = pCopyMob->parts;	/* ROM patch -- Hugin */
    pCurrentMob->imm_flags = pCopyMob->imm_flags;	/* ROM patch -- Hugin */
    pCurrentMob->res_flags = pCopyMob->res_flags;	/* ROM patch -- Hugin */
    pCurrentMob->vuln_flags = pCopyMob->vuln_flags;	/* ROM patch -- Hugin */

    pCurrentMob->off_flags = pCopyMob->off_flags;	/* ROM patch -- Hugin */
    pCurrentMob->size = pCopyMob->size;		/* ROM patch -- Hugin */
    pCurrentMob->ac[AC_PIERCE] = pCopyMob->ac[AC_PIERCE];
    pCurrentMob->ac[AC_BASH] = pCopyMob->ac[AC_BASH];
    pCurrentMob->ac[AC_SLASH] = pCopyMob->ac[AC_SLASH];
    pCurrentMob->ac[AC_EXOTIC] = pCopyMob->ac[AC_EXOTIC];
    pCurrentMob->hit[DICE_NUMBER] = pCopyMob->hit[DICE_NUMBER];
    pCurrentMob->hit[DICE_TYPE] = pCopyMob->hit[DICE_TYPE];
    pCurrentMob->hit[DICE_BONUS] = pCopyMob->hit[DICE_BONUS];
    pCurrentMob->mana[DICE_NUMBER] = pCopyMob->mana[DICE_NUMBER];
    pCurrentMob->mana[DICE_TYPE] = pCopyMob->mana[DICE_TYPE];
    pCurrentMob->mana[DICE_BONUS] = pCopyMob->mana[DICE_BONUS];
    pCurrentMob->damage[DICE_NUMBER] = pCopyMob->damage[DICE_NUMBER];
    pCurrentMob->damage[DICE_TYPE] = pCopyMob->damage[DICE_TYPE];
    pCurrentMob->damage[DICE_NUMBER] = pCopyMob->damage[DICE_NUMBER];
    pCurrentMob->start_pos = pCopyMob->start_pos;
    pCurrentMob->default_pos = pCopyMob->default_pos;
    pCurrentMob->wealth = pCopyMob->wealth;

}

void do_ocopy(CHAR_DATA * ch, char *argument)
{
    OBJ_INDEX_DATA *pCurrentObj;
    OBJ_INDEX_DATA *pCopyObj;
    AFFECT_DATA *pafSrc;
    AFFECT_DATA *pafDest;
    ...
    if (ch->pcdata->security < 9) {
	send_to_char("You do not have high enough security to ocopy.\n\r", ch);
	return;
    }
    argument = one_argument(argument, arg1);
    argument = one_argument(argument, arg2);
    ...
    if (!is_number(arg1)) {
	send_to_char("Please use a valid SOURCE object number to COPY.\n\r", ch);
	return;
    }
    if (!is_number(arg2)) {
	send_to_char("Please use a valid TARGET object to copy TO.\n\r", ch);
	return;
    }
    value = atoi(arg1);
    value2 = atoi(arg2);
    ...
    if (!get_obj_index(value)) {
	send_to_char("SOURCE object you are trying to copy does not exist. \n\r", ch);
	return;
    }
    if (!get_obj_index(value2)) {
	send_to_char("TARGET object you are copying TO does not exist. \n\r", ch);
	return;
    }

    pCurrentObj = get_obj_index(value2);
    pCopyObj = get_obj_index(value);

    pCurrentObj->name = str_dup(pCopyObj->name);
    pCurrentObj->short_descr = str_dup(pCopyObj->short_descr);
    pCurrentObj->material = str_dup(pCopyObj->material);
    pCurrentObj->description = str_dup(pCopyObj->description);

    pCurrentObj->new_format = pCopyObj->new_format;
    pCurrentObj->level = pCopyObj->level;
    pCurrentObj->item_type = pCopyObj->item_type;
    pCurrentObj->wear_flags = pCopyObj->wear_flags;
    pCurrentObj->extra_flags = pCopyObj->extra_flags;
    pCurrentObj->condition = pCopyObj->condition;
    pCurrentObj->weight = pCopyObj->weight;
    pCurrentObj->cost = pCopyObj->cost;
    pCurrentObj->value[0] = pCopyObj->value[0];
    pCurrentObj->value[1] = pCopyObj->value[1];
    pCurrentObj->value[2] = pCopyObj->value[2];
    pCurrentObj->value[3] = pCopyObj->value[3];
    pCurrentObj->value[4] = pCopyObj->value[4];

    //do the affects
    //pafSrc    pafDest

    for (pafSrc = pCopyObj->affected; pafSrc != NULL; pafSrc = pafSrc->next) {
        pafDest = new_affect();
        pafDest->where = pafSrc->where;
	pafDest->type = pafSrc->type;
	pafDest->level = pafSrc->level;
	pafDest->duration = pafSrc->duration;
	pafDest->location = pafSrc->location;
	pafDest->modifier = pafSrc->modifier;
	pafDest->bitvector = pafSrc->bitvector;
	pafDest->next = pCurrentObj->affected;
	pCurrentObj->affected = pafDest;
    }
}
...
void show_obj_values(CHAR_DATA * ch, OBJ_INDEX_DATA * obj)
{
    ...
    case ITEM_FLAG:
	sprintf(buf, "[v0] Flag:  [%s]\n\r", kingdom_table[obj->value[0]].name);
	send_to_char(buf, ch);
	break;
	...
	sprintf(buf,
		...
		obj->value[2],
		obj->value[3],
		flag_string(shield_class, obj->value[4]),
		obj->value[5]);
    send_to_char(buf,ch);
    break;          
...
bool set_obj_values(CHAR_DATA * ch, OBJ_INDEX_DATA * pObj, int value_num, char *argument)
{
    ...
    case ITEM_FLAG:
	if (value_num == 0 && atoi(argument) >= 0 && atoi(argument) <= 3) {
	    send_to_char("Flag side set.\n\r\n\r", ch);
	    pObj->value[0] = atoi(argument);
	} else {
	    do_help(ch, "ITEM_FLAG");
	    return FALSE;
	}
	break;
...
/* ARMOR for ROM: */
	...
	case 3:
	    ...
	    break;
	case 4:
		send_to_char("CHANCE PERCENT SET.\n\r\n\r",ch);
		pObj->value[5] = atoi(argument);
		break;
	case 5:
		send_to_char("SHIELD FLAG SET.\n\r\n\r",ch);
		pObj->value[4] ^= (flag_value(shield_class, argument) != NO_FLAG
			       ? flag_value(shield_class, argument) : 0);
		break;
	}
	break;
...
case ITEM_PARCHMENT:
	...
	case 0:
	    ...
	    break;
	case 1:
	    send_to_char("NUMBER OF TIMES SET.\n\r\n\r", ch);
	    pObj->value[1] = atoi(argument);
	    break;
	}
	break;
...
MEDIT(medit_level)
{
    ...
    if (atoi(argument) > 58) {
	send_to_char("You cant set the level above 58.\n\r", ch);
	return FALSE;
    }
