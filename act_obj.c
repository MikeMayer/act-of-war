...
int find_sdoor args((CHAR_DATA * ch, char *arg, int vnum));
...
void get_obj(CHAR_DATA * ch, OBJ_DATA * obj, OBJ_DATA * container)
{
    ...
    if (!CAN_WEAR(obj, ITEM_TAKE) || obj->item_type == ITEM_FLAG) {
...
void do_withdraw(CHAR_DATA * ch, char *argument)
{
    ...
    /* find out what */
    if (arg == '\0') {
	send_to_char("Withdraw what item from storage?\n\r", ch);
	return;
    }
    if (is_number(arg)) {
	int amount;
	...
	if (!str_cmp(arg, "coins") || !str_cmp(arg, "coin")
	    || !str_cmp(arg, "silver")) {
	    if (ch->banked_silver < amount) {
		send_to_char("You don't have that much silver.\n\r", ch);
		return;
	    }
	    ch->silver += amount;
	    ch->banked_silver -= amount;
	    send_to_char("You withdraw some silver.\n\r", ch);
	} else {
	    if (ch->banked_gold < amount) {
		send_to_char("You don't have that much gold.\n\r", ch);
		return;
	    }
	    ch->gold += amount;
	    ch->banked_gold -= amount;
	    send_to_char("You withdraw some gold.\n\r", ch);
	}
	return;
    }
    obj = get_obj_list(ch, arg, ch->storage);

    if (obj == NULL) {
	send_to_char("You don't see that item in your storage box.\n\r", ch);
	return;
    }
    obj_from_storage(ch, obj);
    obj_to_char(obj, ch);
    send_to_char("You get it from storage.\n\r", ch);
}
...
void do_store(CHAR_DATA * ch, char *argument)
{
	...
	if (!str_cmp(arg, "coins") || !str_cmp(arg, "coin")
	    ...
	    ch->silver -= amount;
	    silver = amount;
	    ch->banked_silver += silver;
	    send_to_char("You deposit some silver.\n\r", ch);

	    if ((ch->banked_silver / 100) >= 1) {
		ch->banked_gold += ch->banked_silver / 100;
		ch->banked_silver %= 100;
		send_to_char("Some silver has been exchanged into gold.\n\r", ch);
	    }
	} else {
	    if (ch->gold < amount) {
		send_to_char("You don't have that much gold.\n\r", ch);
	    ...
	    gold = amount;
	    ch->banked_gold += gold;
	    send_to_char("You deposit some gold.\n\r", ch);
	}

	return;
    }
    count = 0;
    for (obj_max = ch->storage; obj_max != NULL; obj_max = obj_max->next_content) {
	if (count >= MAX_STORE_OBJ) {
	    send_to_char("Sorry, you can't fit anymore into your box.\n\r", ch);
	    return;
	}
	count++;
    }
	...
	send_to_char("You can't deposit this item its too valuable to you.\n\r", ch);
	return;
    }
    if (is_name("restring", obj->name) || is_name("unique", obj->name) ) {
	send_to_char("Depo a unique or a restring? NEVER! You love it too much!\n\r", ch);
	return;
    }

    if (obj->timer > 0) {
	send_to_char("You can't store that!\n\r", ch);
	return;
    }
    if (obj->contains != NULL) {
	send_to_char("Empty its contents first.\n\r", ch);
	return;
    }
    /* assumed to have found the item */
    obj_from_char(obj);
    obj_to_storage(obj, ch);
    send_to_char("You deposit it in your storage box.\n\r", ch);
...
void do_get(CHAR_DATA * ch, char *argument)
{
	...
	if (str_cmp(arg1, "all") && str_prefix("all.", arg1)) {
	    ...
	    if (obj == NULL/* || obj->pIndexData->item_type == ITEM_SWITCH */) {
		...
		if ((arg1[3] == '\0' || is_name(&arg1[4], obj->name))
		    && can_see_obj(ch, obj)/* && obj->pIndexData->item_type != ITEM_SWITCH */) {
		    found = TRUE;
		    get_obj(ch, obj, NULL);
		}
	    }
...
void do_put(CHAR_DATA * ch, char *argument)
{
	...
	if (IS_OBJ_STAT(obj, ITEM_NOBANK)) {
	    send_to_char("You can't put relics in any kind of container.\n\r", ch);
	    return;
	}
...
void do_give(CHAR_DATA * ch, char *argument)
{
	...
	/*
	 * Can't give ld chars money
	 */

	if (victim->desc == NULL && !IS_NPC(victim)) {
	    send_to_char("Sorry, they can't even keep up with thier link.\n\r", ch);
	    return;
	}
	...
	if (    ( quest || auto_quest ) 
       ...
       /* Then ch has recovered the quest object! */
           sprintf( buf, "Oh! %s you found %s {yfor me! Thank You!",
           ...
           if ( !IS_NPC(ch) )
           {
               ...
               if ( obj->value[0] > 0 )
               {
                  ...
                  send_to_char( buf, ch );
                  ch->questpoints += obj->value[0];
               }

               if ( obj->value[2] > 0 )
               {
                  sprintf( buf, "You receive %d gold!\n\r", obj->value[2] );
                  send_to_char( buf, ch );
                  ch->gold += obj->value[2];
               }
			   if (ch->level < LVL_MOBXPCAP)
			   {  
					if ( obj->value[1] > 0 )
					{
						sprintf( buf, "You receive %d exp!\n\r", obj->value[1] );
						send_to_char( buf, ch);
...
void wear_obj(CHAR_DATA * ch, OBJ_DATA * obj, bool fReplace)
{
     ...
     */
    if (obj->item_type == ITEM_LIGHT || obj->item_type == ITEM_DARK) {
...
/*
	if (IS_WEAPON_STAT(wield, WEAPON_TWO_HANDS)) 	{
		send_to_char ("You cannot used a shield while using a two handed weapon.\n\r", ch);
		return;
	}	    
*/	    
...
void do_sacrifice(CHAR_DATA * ch, char *argument)
{
	...
	act("$n offers $mself to AoW, who graciously declines.",
	    ch, NULL, NULL, TO_ROOM);
	send_to_char(
			"AoW appreciates your offer and may accept it later.\n\r", ch);
	return;
    }
	...
	if (obj->contains) {
	    send_to_char(
			    "Remove the items from the players corpse first.\n\r", ch);
	    return;
	}
    }
	if (is_name("restring", obj->name) || is_name("unique", obj->name) ) {
	send_to_char("Doing this would surelly make someone somewhere mad...\n\r", ch);
	return;
    }
    ...
    if (IS_AFFECTED(ch, AFF_NOQUIT) && obj->item_type != ITEM_CORPSE_NPC) {
	send_to_char("At this time the Gods do not accept your sacrifice.\n\r", ch);
	return;
    }
    ...
    act("$n sacrifices $p to AoW.", ch, obj, NULL, TO_ROOM);
...
if ((get_skill(ch, gsn_staves) == 0
	|| ch->level < skill_table[gsn_staves].skill_level[ch->class])) {
	send_to_char("Huh?\n\r", ch);
	return;
    }
...
if ((get_skill(ch, gsn_wands) == 0
	|| ch->level < skill_table[gsn_wands].skill_level[ch->class])) {
	send_to_char("Huh?\n\r", ch);
	return;
    }
...
if ((get_skill(ch, gsn_steal) == 0
    ...
    if (((ch->level + 40 < victim->level || ch->level - 40 > victim->level)
	...
	if (!IS_NPC(ch)) {
		...
		if (!IS_SET(ch->act, PLR_THIEF) && GET_SIDE(ch) == GET_SIDE(victim) && ch->side != SIDE_UM) {
		    SET_BIT(ch->act, PLR_THIEF);
		    
                if (ch->side != SIDE_UM)
		    send_to_char("{Y*** You are now a CRIMINAL!! ***{x\n\r", ch);
		    
		    if (ch->level > 1)
			save_char_obj(ch);
                if (ch->side != SIDE_UM)
                {
		    sprintf(buf, "%s was caught stealing!!\n\r", ch->name);
		    news(buf, ch->side);
                }
		}
	    }
...
void do_repair(CHAR_DATA * ch, char *argument)
{
    ...
    // Check if the shop keeper can actually perform repairs
    for (iTrade = 0; iTrade < MAX_TRADE; iTrade++) {

	if (pShop->buy_type[iTrade] == ITEM_WEAPON)
	    isValid = TRUE;
	if (pShop->buy_type[iTrade] == ITEM_ARMOR)
	    isValid = TRUE;
    }

    if (isValid == FALSE) {

	act("$n tells you 'I can't do repair work here.'",
    ...
    cost = obj->pIndexData->cost;

    if ((obj->item_type != ITEM_ARMOR && obj->item_type != ITEM_WEAPON)) {
	act("$n tells you 'I can't repair that.'",
	    keeper, NULL, ch, TO_VICT);
	ch->reply = keeper;
	return;
    }
    cost = number_range(cost, cost + 200);
    ...
    if (obj->condition >= 95) {
	act("$n tells you 'Sorry, $p is already in perfect condition. '",
	    keeper, obj, ch, TO_VICT);
	ch->reply = keeper;
	return;
    }
    if ((ch->silver + ch->gold * 100) < cost) {
	sprintf(buf, "$n tells you 'You can't afford to repair $p, it will cost you %d gold %d silver.'",
		cost / 100, cost % 100
	    );
    ...
    act("$n repairs $p. Your amazed at the speed at which the repair is completed.",
	ch, obj, NULL, TO_ROOM);
    sprintf(buf, "You repair $p for %d gold and %d silver. Your amazed at how fast its completed.",
	    cost / 100, cost % 100);
...
/*Vashna 03-12-01*/
/*Added cost for silver given a bug that allowed people to repair*/
/*items for free while they had a constant value of silver.*/
...
/*Vashna 03-12-01*/
/*New conditional that will reset value of silver to zero*/
/*if it were to become negative.*/
...
/*      if (ch->in_room->vnum == 20222)
   pRoomIndexNext = get_room_index(20221);
   else */
   ...
   do_buy_mount(ch, arg);/*
	...
	/*
	   ...
	   {
	   send_to_char(
	   "You're not powerful enough to master this .\n\r", ch );
	   return;
	   }
	 */
...
//if(pet->pIndexData->vnum == 14011)
  //  {
//	sprintf(log_buf, "BOOBY: %s bought shadowdragon", (IS_NPC(ch) ? ch->short_descr : ch->name));
//	log_string(log_buf);
//    }
	...
	if (number > 1) {
	    ...
	    sprintf(buf, "You buy $p[%d] for %d gold and %d silver.",
		    number, (cost / 100) * number, (cost % 100) * number);
	    act(buf, ch, obj, NULL, TO_CHAR);
	} else {
	    act("$n buys $p.", ch, obj, NULL, TO_ROOM);
	    sprintf(buf, "You buy $p for %d gold and %d silver.",
		    cost / 100, cost % 100);
	    act(buf, ch, obj, NULL, TO_CHAR);
	}
...
void do_list(CHAR_DATA * ch, char *argument)
{
	...
	for (pet = pRoomIndexNext->people; pet; pet = pet->next_in_room) {
		...
		sprintf(buf, "[%8dgp %8dsp]  %s\n\r",
		/*pet->level, */
			(10 * pet->level * pet->level) / 100,
			(10 * pet->level * pet->level) % 100,
			pet->short_descr);
		send_to_char(buf, ch);
	    }
	...
	for (obj = keeper->carrying; obj; obj = obj->next_content) {
		...
		if (IS_OBJ_STAT(obj, ITEM_INVENTORY))
		    sprintf(buf, "[%3dg %5ds --] %s\n\r",
		    /*obj->level, */ cost / 100, cost % 100, obj->short_descr);
		else {
		    count = 1;
		    ...
		    sprintf(buf, "[%3dg %5ds %2d] %s\n\r",
		    /*obj->level, */ cost / 100, cost % 100, count, obj->short_descr);
		}
		send_to_char(buf, ch);
	    }
	}
...
void do_sell(CHAR_DATA * ch, char *argument)
{
    ...
    //Commented out by Amos. victim is undefined, generating a error.
   /* if (!IS_SET(victim->act, ACT_UNLIMITEDCURRENCY))
    {
...
/* wear object as a secondary weapon */
{
	...
	send_to_char("Why not use a shield instead?\n\r", ch);
	return;
    }
...
void do_raid(CHAR_DATA * ch, char *argument)
{
    ...
    for (obj = ch->in_room->contents; obj != NULL; obj = obj_next) {
	obj_next = obj->next_content;
	if (obj->item_type == ITEM_FLAG && can_see_obj(ch, obj) && ch->side != obj->value[0]) {
	    found = TRUE;
	    break;
	}
    }
    ...
    if (!found) {
	send_to_char("Jeez, there isn't even an enemy flag in the room.  Go figure...\n\r", ch);
	return;
    }
    ...
    if (obj->value[0] == SIDE_TIER)
	main_warscore->tier_been_raided++;
    if (obj->value[0] == SIDE_KAEL)
	main_warscore->kael_been_raided++;
    if (obj->value[0] == SIDE_UM)
	main_warscore->um_been_raided++;
    if (obj->value[0] == SIDE_CAIRN)
	main_warscore->cairn_been_raided++;
    ch->raids++;
    save_char_obj(ch);
    save_warscore();

    act("$n sets the $p on fire and gives a cry of victory!\n\r", ch, obj, NULL, TO_ROOM);
    act("You set the $p on fire and give a cry of victory!\n\r", ch, obj, NULL, TO_CHAR);

    for (i = 0; i < MAX_KINGDOM; i++) {
	sprintf(buf, " Raid!!  %s has been raided", kingdom_table[obj->value[0]].name);
	if (ch->side == i) {
	    strcat(buf, " by ");
	    PERS(ch, ch, pers_buf);
	    strcat(buf, pers_buf);
...
void do_char_restring(CHAR_DATA * ch, char *argument)
{
    char arg1[MAX_INPUT_LENGTH];
    char arg2[MAX_INPUT_LENGTH];
    char buf[MAX_STRING_LENGTH];
...
if(arg1[0] == '\0' || arg2[0] == '\0')
    {
    send_to_char("What item do you want to restring to what?\n\r", ch);
    return;
    }
if ((obj = get_obj_carry(ch, arg1, ch)) == NULL)
    {
    ...
    token = get_eq_char(ch, WEAR_HOLD);
    if ( token == NULL || str_prefix("111restring", token->name))
    {
    send_to_char("You must hold the token in order to restring an item.\n\r", ch);
    return;
    }
sprintf(buf2, "Restring %s %s", ch->name, obj->name);

free_string(obj->name);
obj->name = str_dup(buf2);
free_string(obj->short_descr);
sprintf(buf, "%s{x", arg2);
