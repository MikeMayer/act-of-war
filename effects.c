...
void cold_effect(void *vo, int level, int dam, int target)
{
	...
	/* chill touch effect */
	if (!saves_spell(level / 4 + dam / 20, victim, DAM_COLD,0)) {
	    AFFECT_DATA af;
...
void poison_effect(void *vo, int level, int dam, int target)
{
	...
	/* chance of poisoning */
	if (!saves_spell(level / 4 + dam / 20, victim, DAM_POISON,0)) {
	    AFFECT_DATA af;
...
void shock_effect(void *vo, int level, int dam, int target)
{
	...
	/* daze and confused? */
	if (!saves_spell(level / 4 + dam / 20, victim, DAM_LIGHTNING,0)) {
