...
void calculate_ac(CHAR_DATA * ch)
{
    int loc, mod, t, i;
    OBJ_DATA *obj;
    AFFECT_DATA *af;
    ...
    // Personal Affects now need added to AC
...
void char_to_room(CHAR_DATA * ch, ROOM_INDEX_DATA * pRoomIndex)
{
	...
	if ((room =
	     get_room_index(kingdom_table[ch->side].recall_room_num)) != NULL)
...
{
    obj->next_content = ch->storage;
    ch->storage = obj;
    obj->in_room = NULL;
}

void obj_from_storage(CHAR_DATA * ch, OBJ_DATA * obj)
{
    OBJ_DATA *prev;
    ...
    if (ch->storage == obj) {
	ch->storage = obj->next_content;
    } else {

	for (prev = ch->storage; prev != NULL; prev = prev->next_content) {
...
int apply_ac(OBJ_DATA * obj, int iWear, int type)
{
	...
	value = obj->value[type] / 2;
    else if (obj->condition <= 30)
	value = obj->value[type] / 4;

    switch (iWear) {
    case WEAR_BODY:
	return 2 * obj->value[type] - value;
    case WEAR_HEAD:
	return 1 * obj->value[type] - value;
    case WEAR_LEGS:
	return 1 * obj->value[type] - value;
    case WEAR_FEET:
	return obj->value[type] - value;
    case WEAR_HANDS:
	return obj->value[type] - value;
    case WEAR_ARMS:
	return obj->value[type] - value;
...
void equip_char(CHAR_DATA * ch, OBJ_DATA * obj, int iWear)
{
    ...
    /*
       ...
       if( (CAN_WEAR(obj, ITEM_TIER ) && GET_SIDE(ch) != SIDE_TIER)
       ||  (CAN_WEAR(obj, ITEM_KAEL ) && GET_SIDE(ch) != SIDE_KAEL)
       ||  (CAN_WEAR(obj, ITEM_UM   ) && GET_SIDE(ch) != SIDE_UM)
       ||  (CAN_WEAR(obj, ITEM_CAIRN) && GET_SIDE(ch) != SIDE_CAIRN)
       )
       {
       act( "That belongs to the enemy. You drop $p.", ch, obj, NULL, TO_CHAR );
       act( "$n drops $p because it belongs to the enemy.",  ch, obj, NULL, TO_ROOM );
       obj_from_char( obj );
       obj_to_room( obj, ch->in_room );
       return;
       }
...
void extract_obj(OBJ_DATA * obj)
{
    ...
    OBJ_DATA *obj_next; 	//ROOM_INDEX_DATA *drop_room; 
    extern bool quest;     extern OBJ_DATA *quest_object;  	if ( ( obj == quest_object ) && quest )     return; 
    if (obj->in_room != NULL)
	obj_from_room(obj);
...
void extract_char_keepeq(CHAR_DATA * ch, bool fPull)
{
    CHAR_DATA *wch;
    ...
    if (!fPull) {
	char_to_room(ch, get_room_index(kingdom_table[ch->side].die_room_num));
	return;
    }
...
void extract_char(CHAR_DATA * ch, bool fPull)
{
    ...
    if (ch == quest_target) 		quest_target = NULL;  	
    if (ch == quest_mob) 		quest_mob = NULL; 
    /* nuke_pets(ch); */
    ch->pet = NULL;
    ...
    if (IS_NPC(ch)) { //BIG IFFFFFFF. MAYBE THIS WILL FIX MY CORPSE DROP CHANCE! -Izlude 8/24/04
...
CHAR_DATA *get_player_world_not_mob(CHAR_DATA * ch, char *argument)
{
    char arg[MAX_INPUT_LENGTH];
    CHAR_DATA *wch;
    int number;
    ...
    for (wch = char_list; wch != NULL; wch = wch->next) {
	if (wch->in_room == NULL || !can_see(ch, wch)
	    || !is_name(arg, wch->name) || IS_NPC(wch))
	    continue;
	if (++count == number)
	    return wch;
    }
...
CHAR_DATA *get_char_worldf(CHAR_DATA * ch, char *argument)
{
    char arg[MAX_INPUT_LENGTH];
    CHAR_DATA *wch;
    int number;
...
bool room_is_dark(ROOM_INDEX_DATA * pRoomIndex)
{
    ...
    int light = 1;  //if this ends up above 0 then its light
...
{
	if ((obj = get_eq_char(rch, WEAR_LIGHT)) != NULL &&
	    obj->item_type == ITEM_LIGHT &&
	    obj->value[2] != 0)
	    light++;
...
bool can_see_room(CHAR_DATA * ch, ROOM_INDEX_DATA * pRoomIndex)
{
    ...
    if (IS_SET(pRoomIndex->room_flags, ROOM_LEGENDS_ONLY)
	&& ch->level < LEGEND_LEVEL && !IS_IMMORTAL(ch))
	return FALSE;

    if (!IS_IMMORTAL(ch) &&
	pRoomIndex->clan &&
	ch->clan != pRoomIndex->clan &&
	ch->side == clan_table[pRoomIndex->clan].side)
	return FALSE;
...
bool can_see_obj(CHAR_DATA * ch, OBJ_DATA * obj)
{
	...
	((!IS_AFFECTED(ch, AFF_INFRARED)) &&
	 (!IS_SET(ch->vuln_flags, VULN_SUN)
	  && ((obje = get_eq_char(ch, WEAR_LIGHT)) == NULL ||
	      obje->item_type != ITEM_LIGHT ||
	      obje->value[2] == 0))
	)
	)
	return FALSE;
...
char *affect_bit_name(int vector)
{
    ...
    if (vector & AFF_REFLECT)
	strcat(buf, " reflect");
	if (vector & AFF_CHAOTIC)
	strcat(buf, " chaotic_aura");
	if (vector & AFF_DEFLECT)
	strcat(buf, " deflective_aura");
    if (vector & AFF_DETECT_ALIGNMENT)
	strcat(buf, " detect_alignment");
 ...
 */
    if (vector & AFF_UNDEAD)
	strcat(buf, " undead");
    if (vector & AFF_BLUFFED)
	strcat(buf, " bluffed");
    if (vector & AFF_IMM_FROZEN)
	strcat(buf, " frozen");
    if (vector & AFF_LAPSE)
	strcat(buf, " lapsed");
    if (vector & AFF_SUSPEND)
...
char *act_bit_name(int act_flags)
{
	...
	if (act_flags & ACT_PET)
	    strcat(buf, " pet");
	if (act_flags & ACT_MOUNT)
	    strcat(buf, " mount");
	if (act_flags & ACT_NOFLEE)
	    strcat(buf, " noflee");
	...
	if (act_flags & PLR_NOCLAN)
	    strcat(buf, " no_clan");
	if (act_flags & PLR_CLANLEADER)
	    strcat(buf, " leader");
...
char *comm_bit_name(int comm_flags)
{
    ...
    if (comm_flags & COMM_NOTRIVIA)
	strcat(buf, " no_trivia");
...
char *res_bit_name(int imm_flags) {     static char buf[512];      buf[0] = '\0';     if (imm_flags & RES_SUMMON) 	strcat(buf, " summon");     if (imm_flags & RES_CHARM) 	strcat(buf, " charm"); 	if (imm_flags & RES_MAGIC) 	strcat(buf, " magic");     if (imm_flags & RES_WEAPON) 	strcat(buf, " weapons"); 	if (imm_flags & RES_BASH) 	strcat(buf, " bash"); 	if (imm_flags & RES_PIERCE) 	strcat(buf, " pierce"); 	if (imm_flags & RES_SLASH) 	strcat(buf, " slash"); 	if (imm_flags & RES_FIRE) 	strcat(buf, " fire"); 	if (imm_flags & RES_COLD) 	strcat(buf, " cold"); 	if (imm_flags & RES_LIGHTNING) 	strcat(buf, " lightning"); 	if (imm_flags & RES_ACID) 	strcat(buf, " acid"); 	if (imm_flags & RES_POISON) 	strcat(buf, " poison"); 	if (imm_flags & RES_NEGATIVE) 	strcat(buf, " negative"); 	if (imm_flags & RES_HOLY) 	strcat(buf, " holy"); 	if (imm_flags & RES_ENERGY) 	strcat(buf, " energy"); 	if (imm_flags & RES_MENTAL) 	strcat(buf, " mental"); 	if (imm_flags & RES_DISEASE) 	strcat(buf, " disease"); 	if (imm_flags & RES_DROWNING) 	strcat(buf, " drowning"); 	if (imm_flags & RES_LIGHT) 	strcat(buf, " light"); 	if (imm_flags & RES_SOUND) 	strcat(buf, " sound"); 	if (imm_flags & RES_WOOD) 	strcat(buf, " wood"); 	if (imm_flags & RES_SILVER) 	strcat(buf, " silver"); 	if (imm_flags & RES_IRON) 	strcat(buf, " iron");      return (buf[0] != '\0') ? buf + 1 : "none"; }  char *vuln_bit_name(int imm_flags) {     static char buf[512];      buf[0] = '\0';     if (imm_flags & VULN_SUMMON) 	strcat(buf, " summon");     if (imm_flags & VULN_CHARM) 	strcat(buf, " charm"); 	if (imm_flags & VULN_MAGIC) 	strcat(buf, " magic");     if (imm_flags & VULN_WEAPON) 	strcat(buf, " weapons"); 	if (imm_flags & VULN_BASH) 	strcat(buf, " bash"); 	if (imm_flags & VULN_PIERCE) 	strcat(buf, " pierce"); 	if (imm_flags & VULN_SLASH) 	strcat(buf, " slash"); 	if (imm_flags & VULN_FIRE) 	strcat(buf, " fire"); 	if (imm_flags & VULN_COLD) 	strcat(buf, " cold"); 	if (imm_flags & VULN_LIGHTNING) 	
strcat(buf, " lightning"); 	if (imm_flags & VULN_ACID) 	strcat(buf, " acid"); 	if (imm_flags & VULN_POISON) 	strcat(buf, " poison"); 	if (imm_flags & VULN_NEGATIVE) 	strcat(buf, " negative"); 	if (imm_flags & VULN_HOLY) 	strcat(buf, " holy"); 	if (imm_flags & VULN_ENERGY) 	strcat(buf, " energy"); 	if (imm_flags & VULN_MENTAL) 	strcat(buf, " mental"); 	if (imm_flags & VULN_DISEASE) 	strcat(buf, " disease"); 	if (imm_flags & VULN_DROWNING) 	strcat(buf, " drowning"); 	if (imm_flags & VULN_LIGHT) 	strcat(buf, " light"); 	if (imm_flags & VULN_SOUND) 	strcat(buf, " sound"); 	if (imm_flags & VULN_SUN) 	strcat(buf, " sun"); 	if (imm_flags & VULN_WOOD) 	strcat(buf, " wood"); 	if (imm_flags & VULN_SILVER) 	strcat(buf, " silver"); 	if (imm_flags & VULN_IRON) 	strcat(buf, " iron");      return (buf[0] != '\0') ? buf + 1 : "none"; } 
