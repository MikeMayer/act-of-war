...
void do_gain(CHAR_DATA * ch, char *argument)
{
    ...
    CHAR_DATA *trainer;
    int gn = 0, sn = 0;
//    char * spec;  commented out to remove warning: unused variable `spec'
	...
	act("$N tells you I can no longer do that.'",
    ...
    if (!str_prefix(arg, "points")) {
         act("$N tells you 'You cannot do that anymore.'", ch, NULL, trainer, TO_CHAR);	
      /*if (ch->train < 2) {
	    act("$N tells you 'You are not yet ready.'",
		ch, NULL, trainer, TO_CHAR);
	    return;
...
if(inc > 1500)
    ...
    /*Sanity check follows, need to change sometime. */
    test = expl * pc_race_table[ch->race].class_mult[ch->class] / 100;
    if (test <= 0)
	test = 1000000;
    return test;
}
...
//	|| skill_table[sn].rating[ch->class] == 0
    ...
    chance /= (multiplier
	       * (skill_table[sn].rating[ch->class] > 0 ? skill_table[sn].rating[ch->class] : 6)
	       * 4);
    chance += ch->level;
	...
	if ((ch->side == 4) && ((ch->in_room->vnum /100 == 280)
	|| (ch->in_room->vnum/100 == 281) 
	|| (ch->in_room->vnum/100 == 282) 
	|| (ch->in_room->vnum/100 == 283)
	|| (ch->in_room->vnum/100 == 144) 
	|| (ch->in_room->vnum/100 == 145) ))
	return;
	...
	chance += 80;
	if (vch != NULL && !IS_NPC(vch)){ //20% + bonus to fighting a PC, 33% bonus if that PC is also not your kingdom.
	chance += 20;
	if (vch->side != ch->side)
	chance += 33;
}
...
int group_lookup(const char *name)
{
  ...
  int group_lookup_exact(const char *name)
{
    int gn;
