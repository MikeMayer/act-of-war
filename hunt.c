...
CHAR_DATA *get_char_area args((CHAR_DATA * ch, char *arg));
...
void push_dir(CHAR_DATA * ch, int dir)
{
    DIRECTION_STACK *newdir;

    newdir = malloc(sizeof(DIRECTION_STACK));

    newdir->dir = dir;
    newdir->next = ch->dir_stack;
    ch->dir_stack = newdir;
}

int pop_dir(CHAR_DATA * ch)
{
    DIRECTION_STACK *tempdir;
    int popvalue;

    tempdir = ch->dir_stack;
    popvalue = ch->dir_stack->dir;
    ch->dir_stack = ch->dir_stack->next;
    free(tempdir);
    return popvalue;
}
...
if ((get_skill(ch, gsn_hunt) == 0
	...
	&& ch->level < skill_table[gsn_hunt].skill_level[ch->class]) {
	send_to_char(
			"You have no idea how to hunt or track.\n\r", ch);
	return;
    }
    if (arg[0] == '\0') {
    ...
    act("$n carefully looks for tracks.", ch, NULL, NULL, TO_ROOM);
     ...
     */
    sprintf(buf, "You see tracks of $N leading %s from here.", dir_name[direction]);
...
if ((get_skill(ch, gsn_chase) == 0
	|| ch->level < skill_table[gsn_chase].skill_level[ch->class])) {
	send_to_char("Huh?\n\r", ch);
	return;
    }
	...
	&& ch->level < skill_table[gsn_chase].skill_level[ch->class]) {
	send_to_char(
			"You have no idea how to chase.\n\r", ch);
	return;
    }
    if (arg[0] == '\0') {
	send_to_char("Whom are you trying to chase?\n\r", ch);
	return;
    }
    /* only imps can chase to different areas */
    fArea = (get_trust(ch) < MAX_LEVEL);
	...
	send_to_char("You're too exhausted to chase anyone!\n\r", ch);
	return;
    }

    act("$n carefully looks for tracks.", ch, NULL, NULL, TO_ROOM);

    directionNum = direction = find_path(ch->in_room->vnum, victim->in_room->vnum, ch, -40000, fArea);
     ...
     */
    if ((IS_NPC(ch) && number_percent() > 75)	/* NPC @ 25% */
	||(!IS_NPC(ch) && number_percent() >	/* PC @ norm. 10% added failure for chase */
	   ch->pcdata->learned[gsn_chase] -10   )) {
	do {
	    direction = number_door();
	}
 ...
 move_char(ch, direction, FALSE);
    ...
    check_improve(ch, gsn_chase, TRUE, 1);
...
void hunt_victim(CHAR_DATA * ch)
{
    ...
    if (ch->hunting == NULL && ch->dir_stack == NULL)
	return;
	...
	while ((ch->in_room->exit[dir] == NULL) || (ch->in_room->exit[dir]->u1.to_room == NULL));
    }
    if (IS_SET(ch->in_room->exit[dir]->exit_info, EX_CLOSED)) {
	do_open(ch, (char *) dir_name[dir]);
	return;
    }
    if (IS_SET(ch->in_room->exit[dir]->u1.to_room->room_flags, ROOM_NO_MOB)) {
	ch->hunting = NULL;
	return;
    } else 
    {
	move_char(ch, dir, FALSE);
	push_dir(ch, reverse_dir(dir));
    }
