...
void song_update(void)
{
	...
	if (obj->value[0] < 0) {
	    sprintf(buf, "$p starts playing {W%s, %s{x.",
...
//	sprintf(buf, "$p sings: '{y%s{x'", line);
	sprintf(buf, "'{y%s{x'", line);
	if (room->people != NULL)
	    act(buf, room->people, obj, NULL, TO_ALL);
    }
}
...
void do_play(CHAR_DATA * ch, char *argument)
{
	...
	sprintf(buf, "\n\r%s has the following songs available:\n\r\n\r",
		juke->short_descr);
