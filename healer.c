...
*/
	...
	send_to_char(" {w blind{x:    cure blindness	{w20{x gold\n\r", ch);
	send_to_char("  {wdisease{x:  cure disease	{w15{x gold\n\r", ch);
	send_to_char("  {wpoison{x:   cure poison      	{w25{x gold\n\r",
ch);
	send_to_char("  {wuncurse{x:  remove curse	{w50{x gold\n\r",
ch);
	send_to_char("  {wrefresh{x:  restore movement	{w5{x gold\n\r", ch);
        send_to_char("  {wlight{x:    cure light       	{w15{x gold\n\r",
ch);
        send_to_char("  {wserious{x:  cure serious	{w25{x gold\n\r", ch);
	send_to_char("  {wcritical{x: cure critical	{w50{x gold\n\r", ch);
	send_to_char(" Type {wheal <type>{x to be cured.\n\r", ch);
	return;
    }
